...
def main(argv):
    ...
    if proceed == True:
        ...
        call(['cqlsh', '-f', '/mnt/data/src/bitbucket.org/infrared/models/kafka/consumer/clean_kairos.cql'])
        call(['sudo', 'service', 'kairosdb', 'start'])

    proceed = yes_or_no('Do you wish to reset schema')
    if proceed == True:
        call(['cqlsh', '-f', '/mnt/data/src/bitbucket.org/infrared/models/kafka/consumer/schema.cql'])
        call(['sudo', 'service', 'cassandra', 'restart'])
        print "Waiting 5 secs"
        time.sleep(5)
        call(['tail', '/var/log/cassandra/system.log'])
        print "Updating node details"
        call(['/mnt/data/src/bitbucket.org/infrared/deploy/update-kafka.sh'])
