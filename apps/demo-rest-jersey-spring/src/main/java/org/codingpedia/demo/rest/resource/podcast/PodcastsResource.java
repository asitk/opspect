...
public class PodcastsResource {
  ...
  /**
   * Adds a new resource (podcast) from the given json format (at least title and feed elements are
  ...
  public Response createPodcast(Podcast podcast) throws AppException {
    ...
    return Response.status(Response.Status.CREATED) // 201
        ...
        .header(
            "Location",
            "http://localhost:7779/demo-rest-jersey-spring/podcasts/"
                + String.valueOf(createPodcastId))
        .build();
  }

  /**
   * Adds a new podcast (resource) from "form" (at least title and feed elements are required at the
   * DB level)
   *
   * @param title
  ...
  public Response createPodcastFromApplicationFormURLencoded(
    ...
    return Response.status(Response.Status.CREATED) // 201
        .entity(
            "A new podcast/resource has been created at /demo-rest-jersey-spring/podcasts/"
                + createPodcastid)
        .header(
            "Location",
            "http://localhost:7779/demo-rest-jersey-spring/podcasts/"
  ...
  /**
   * A list of resources (here podcasts) provided in json format will be added to the database.
   *
   * @param podcasts
   * @return
   * @throws AppException
  ...
  public Response createPodcasts(List<Podcast> podcasts) throws AppException {
    ...
    return Response.status(Response.Status.CREATED) // 201
        .entity("List of podcasts was successfully created")
        .build();
  }
  ...
  //	@Produces({ MediaType.APPLICATION_JSON, MediaType.APPLICATION_XML })
  //	@PodcastDetailedView
  //	public Podcast getPodcastById(@PathParam("id") Long id, @QueryParam("detailed") boolean
  // detailed)
  //			throws IOException,	AppException {
  //		Podcast podcastById = podcastService.getPodcastById(id);
  //
  //		return podcastById;
  ////		return Response.status(200)
  ////				.entity(podcastById, detailed ? new Annotation[]{PodcastDetailedView.Factory.get()} : new
  // Annotation[0])
  ////				.header("Access-Control-Allow-Headers", "X-extra-header")
  ////				.allow("OPTIONS").build();
  //	}
  ...
  /**
   * The method offers both Creation and Update resource functionality. If there is no resource yet
   * at the specified location, then a podcast creation is executed and if there is then the
   * resource will be full updated.
   *
   * @param id
   * @param podcast
   * @return
  ...
  public Response putPodcastById(@PathParam("id") Long id, Podcast podcast) throws AppException {
    ...
    if (podcastById == null) {
      ...
      return Response.status(Response.Status.OK)
          // 200
          .entity(
              "The podcast you specified has been fully updated created AT THE LOCATION you"
                  + " specified")
          .header(
              "Location",
              "http://localhost:7779/demo-rest-jersey-spring/podcasts/" + String.valueOf(id))
          .build();
    }
  }
