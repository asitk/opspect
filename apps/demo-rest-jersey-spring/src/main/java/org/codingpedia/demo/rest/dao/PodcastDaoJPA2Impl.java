...
public class PodcastDaoJPA2Impl implements PodcastDao {
  ...
  public List<PodcastEntity> getPodcasts(String orderByInsertionDate) {
    ...
    if (orderByInsertionDate != null) {
      sqlString =
          "SELECT p FROM PodcastEntity p" + " ORDER BY p.insertionDate " + orderByInsertionDate;
    } else {
  ...
  public List<PodcastEntity> getRecentPodcasts(int numberOfDaysToLookBack) {
    ...
    calendar.add(
        Calendar.DATE, -numberOfDaysToLookBack); // substract the number of days to look back
    Date dateToLookBackAfter = calendar.getTime();

    String qlString =
        "SELECT p FROM PodcastEntity p where p.insertionDate > :dateToLookBackAfter ORDER BY"
            + " p.insertionDate DESC";
  ...
  public List<PodcastEntity> getLegacyPodcasts() {
    ...
    TypedQuery<PodcastEntity> query =
        entityManagerLegacy.createQuery(qlString, PodcastEntity.class);
