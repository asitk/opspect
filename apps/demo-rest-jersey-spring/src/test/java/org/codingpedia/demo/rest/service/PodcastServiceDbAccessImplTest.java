...
public class PodcastServiceDbAccessImplTest {
  ...
  public void testCreatePodcast_successful() throws AppException {
    ...
    when(podcastDao.createPodcast(any(PodcastEntity.class)))
        .thenReturn(CREATED_PODCAST_RESOURCE_ID);
    ...
    verify(podcastDao)
        .getPodcastByFeed(
            SOME_FEED); // verifies if the method podcastDao.getPodcastByFeed has been called
    // exactly once with that exact input parameter
    ...
    verify(podcastDao, times(1))
        .getPodcastByFeed(
            anyString()); // verifies if the method podcastDao.getPodcastByFeed has been called
    // exactly once with any string as input
    verify(podcastDao, atLeastOnce())
        .getPodcastByFeed(
            SOME_FEED); // verifies if the method podcastDao.getPodcastByFeed has been called at
    // least once with that exact input parameter
    verify(podcastDao, atLeast(1))
        .getPodcastByFeed(
            SOME_FEED); // verifies if the method podcastDao.getPodcastByFeed has been called at
    // least once with that exact input parameter
    verify(podcastDao, times(1)).createPodcast(any(PodcastEntity.class));
    verify(podcastDao, never())
        .getLegacyPodcastById(
            anyLong()); // verifies the method podcastDao.getLegacyPodcastById has never been called
  ...
  public void testUpdatePartiallyPodcast_successful() throws AppException {
    ...
    verify(podcastDao)
        .getPodcastById(
            SOME_ID); // verifies if the method podcastDao.getPodcastById has been called exactly
    // once with that exact input parameter
