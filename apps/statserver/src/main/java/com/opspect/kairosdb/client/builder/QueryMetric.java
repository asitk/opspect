...
/**
 * Query request for a metric. If a metric is queried by name only then all data points for all tags
 * are returned. You can narrow down the query by adding tags so only data points associated with
 * those tags are returned.
 *
 * <p>Aggregators may be added to the metric. An aggregator performs an operation on the data such
 * as summing or averaging. If multiple aggregators are added, the output of the first is sent to
 * the input of the next, and so forth until all aggregators have been processed, These are
 * processed in the order they were added.
 *
 * <p>
 *
 * <p>The results of the query can be grouped in various ways using a grouper. For example, if you
 * had a metric with a customer tag, the resulting data points could be grouped by the different
 * customers. Multiple groupers can be used so you could, for example, group by tag and value.
 *
 * <p>Note that aggregation is very fast but grouping can slow down the query.
 */
...
public class QueryMetric {
  ...
  /**
   * Add a map of tags. This narrows the query to only show data points associated with the tags'
   * values.
   *
   * @param tags tags to add
   * @return the metric
  ...
  /**
   * Adds a tag with multiple values. This narrows the query to only show data points associated
   * with the tag's values.
   *
   * @param name tag name
  ...
  /**
   * Limits the number of data point returned from the query. The limit is done before aggregators
   * are executed.
   *
  ...
  public boolean equals(Object o) {
    ...
    if (groupers != null ? !groupers.equals(that.groupers) : that.groupers != null) return false;
