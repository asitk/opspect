...
public class ResultsDeserializer implements JsonDeserializer<Results> {
  ...
  public Results deserialize(JsonElement json, Type typeOfT, JsonDeserializationContext context)
    ...
    Map<String, List<String>> tags =
        context.deserialize(tagsElement, new TypeToken<Map<String, List<String>>>() {}.getType());

    // Group_By
    JsonElement group_by = json.getAsJsonObject().get("group_by");
    List<GroupResult> groupResults =
        context.deserialize(group_by, new TypeToken<List<GroupResult>>() {}.getType());
    ...
    if (group_by != null) {
      ...
      checkState(
          dataPointValueClass != null,
          "type: " + type + " is not registered to a custom data type.");
      ...
      for (JsonElement element : array) {
        ...
        dataPoints.add(
            new DataPoint(
                pair.get(0).getAsLong(),
                context.<DataPoint>deserialize(pair.get(1), dataPointValueClass)));
      }
    }
