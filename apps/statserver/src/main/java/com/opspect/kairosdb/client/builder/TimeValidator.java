...
public class TimeValidator {
  ...
  public static void validateEndTimeLaterThanStartTime(
      RelativeTime startTime, RelativeTime endTime) {
    long now = System.currentTimeMillis();
    checkState(
        startTime.getTimeRelativeTo(now) < endTime.getTimeRelativeTo(now),
        "Start time cannot be later than the ending time");
  }

  public static void validateEndTimeLaterThanStartTime(long startTime, RelativeTime endTime) {
    long now = System.currentTimeMillis();
    checkState(
        startTime < endTime.getTimeRelativeTo(now),
        "Start time cannot be later than the ending time");
  }
  ...
  public static void validateEndTimeLaterThanStartTime(RelativeTime startTime, long endTime) {
    long now = System.currentTimeMillis();
    checkState(
        startTime.getTimeRelativeTo(now) < endTime,
        "Start time cannot be later than the ending time");
  }
}
