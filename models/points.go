...
import (
	...
	"bitbucket.org/infrared/util/escape"
)
...
func (p *point) String() string {
	...
	// asit: Do not modify this sequence. The spark parse depends on value being at the end
	// return string(p.Key()) + ",ts=" + strconv.FormatInt(p.UnixNano(), 10) + "," + string(p.fields)

	// seconds
	//return string(p.Key()) +
	//	",ts=" +
	//	strconv.FormatInt(p.UnixNano()/(int64(time.Second)/int64(time.Nanosecond)), 10) +
	//	"," + string(p.fields)

	// milliseconds
	return string(p.Key()) +
		",ts=" +
		strconv.FormatInt(p.UnixNano()/(int64(time.Millisecond)/int64(time.Nanosecond)), 10) +
		"," + string(p.fields)
}
