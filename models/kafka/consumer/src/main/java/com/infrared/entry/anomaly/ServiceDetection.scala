 ...
 */
class ServiceDetection(_cluster_map: Cluster.ClusterMap, _time_range: TimeRange) extends AnomalyTrait {
  ...
  def getUniqServicesByCluster(arows: Array[Metrics.MetricsStats]): Map[(String, String), String] = {
    //Just read any entries from the DB if any. This will be updated the next minute
    val nw_svc_list = NWObject.getServiceListFromDB(cluster_map.customer_id, cluster_map.deployment_id, time_range)
  ...
  def workOnDistribution(_arows: Array[Metrics.MetricsStats]): List[AnomalyCollection] = {
    ...
    val arows = _arows.filter(_.plugin == "procstat").filter(_.classification == "memory_rss")
    ...
    val nm = getMaps(Some(cluster_map.customer_id), Some(cluster_map.deployment_id), Some(cluster_map.cluster_id),
    ...
    arows.groupBy(x => {
      (x.customer_id, x.deployment_id, x.cluster_id, x.host_ip, x.plugin, x.target)
    }).values.foreach(v => {
      ...
      if (qs.severity != sevGrades.last.grade) {
        ...
        val ms = new MetricsStats(time_range.start, z.customer_id, z.deployment_id, z.cluster_id, z.target, z.host_ip, z.host_name,
          "service", "uptime", 0, 0, false, z.val_stats, z.vel_stats, z.tags, z.val_str)
