package com.infrared.entry.rawrecord

import com.google.gson._
import com.infrared.util.{JacksonWrapper, TCPClient, _}
...
case class RawRecord (rectype: Int, plugin: String, classification:String, target  :String, host_ip: String,
                      host_name:String, cluster_id: String, deployment_id : String, customer_id : String,
                      value: String, ts: String, orig_ts: Long, tags: scala.collection.mutable.HashMap[String, String], hinttagv: String)
  extends Serializable
{
  ...
  def getDeltaValue(r1: RawRecord, n: Int): (Int, HashMap[String, Any]) = {
    ...
    val (value1, value2) = {
      if (n == 0) {
        ...
        (r1.tags.getOrElse(deltaValueType, null), r2.tags.getOrElse(deltaValueType, null))
      }
    }
  ...
  def write():Unit = {
    ...
    /**
    if (rectype == RawRecordType.LOG) {
      val (url, text) = toElasticString()
      var url_prefix = "http://localhost:9200/"
      url_prefix = url_prefix + url

      System.out.println("URL: " + url_prefix + " Text: " + text)
      httpClient.Post(url_prefix, text)
      return
    }

    // Handle Procstat
    if (rectype == RawRecordType.SYSTEM && classification == "cmd") {
      System.out.println(toOpenTSDBTelnetString())
      return
    }
