...
import com.infrared.entry.Tokenize._
import com.infrared.entry.infra.Service.ServiceMap
import com.infrared.entry.infra.{Cluster, Node, Service}
...
object NWObject {
  ...
  case class NWSvcDetail(method: String, customer_id: String, deployment_id: String, nw_svc_info_list: List[NWSvcInfo], time_range: TimeRange) extends Serializable {
    override def toString: String = {
      JacksonWrapper.serialize(this)
    }
  }

  case class NWSvcInfo(cluster_id: String, name: String, svc: Int, ipver: Int, proto: Int, port: Int, interface: String) extends Serializable {
    override def toString: String = {
      JacksonWrapper.serialize(this)
    }
  }

  case class ServiceInfo(svc_info: NWSvcInfo, observations: Map[String, String]) extends Serializable {
    override def toString: String = {
      JacksonWrapper.serialize(this)
    }
  }
  ...
  case class RequestStat(req: String, count: Long, ttfb: Long, ttlb: Long) extends Serializable {
    override def toString: String = {
      JacksonWrapper.serialize(this)
    }
  }

  case class NWGraph(to: Node.NodeMap, from: Node.NodeMap, svc_info: NWSvcInfo,
                     avg_rsp_size: Long, avg_ttfb: Long, avg_ttlb: Long,
                     max_ttfb: Long, max_ttlb: Long, error_count: Long, request_count: Long, sent_bytes: Long, recv_bytes: Long,
                     topmost_error_stats: Array[RequestStat], costliest_request_stats: Array[RequestStat],
                     ts: Long, duration: Long) extends Serializable {
    override def toString: String = {
      JacksonWrapper.serialize(this)
    }
  }

  case class NWBinding(to: Node.NodeMap, from: Node.NodeMap, scvInfo: NWSvcInfo) extends Serializable {
    override def toString: String = {
      JacksonWrapper.serialize(this)
    }
  }

  def findValue(arows: Array[Metrics.MetricsStats], classification: String): String = {
    arows.filter(_.classification == classification).head.val_str
  }

  def populateNodeMapFromValue(x: Array[Metrics.MetricsStats], is_src_ip: Boolean): Node.NodeMap = {
    val nm = {
      val ip = {
      ...
      if (ip != x.head.host_ip) {
        val nm = Node.getMaps(Some(x.head.customer_id), Some(x.head.deployment_id), None, Some(ip), None, None)
        if (nm.isEmpty) {
        ...
        new Node.NodeMap(x.head.customer_id, x.head.deployment_id, x.head.cluster_id, ip, x.head.host_name, "unknown", true, 0)
      }
    }
    new Node.NodeMap(nm.customer_id, nm.deployment_id, nm.cluster_id, nm.host_ip, nm.host_name, nm.scope, nm.active, 0l)
  }

  def populateNWSvcByTS(x: Array[Metrics.MetricsStats], nw_svc: List[NWSvcDetail]): NWGraph = {
    val ts = x.head.ts
    ...
    val topmost_error_request_count = findValue(x, "topmost_error_request_count").toDouble.toLong
    val topmost_error_request_str = findValue(x, "topmost_error_request_str")
    val topmost_error_request_ttfb = findValue(x, "topmost_error_request_ttfb").toDouble.toLong
    val costly_request = {
      val cr = new RequestStat(costliest_request_str, 1, max_ttfb, max_ttlb)
      Array(cr)
    }
    val err_request = {
      val er = new RequestStat(topmost_error_request_str, topmost_error_request_count,
        topmost_error_request_ttfb, topmost_error_request_ttfb)
      Array(er)
    }
    val duration = findValue(x, "duration").toDouble.toLong
    val observations = Map[String, String]()
    new NWGraph(to, from, svc_info, avg_rsp_size, avg_ttfb, avg_ttlb, max_ttfb, max_ttlb, err_count,
      req_count, recv_bytes, sent_bytes, err_request, costly_request, x.head.ts, duration)
  }

  def findAppropriateIndex(reqStat: Array[RequestStat], req: String): (Int, Boolean) = {
  ...
  def getNWServices(customer_id: String, deployment_id: String, time_range: TimeRange): List[NWSvcDetail] = {
    val nw_svc_details = {
      val key = s"svc_${customer_id},${deployment_id}"
      if (MyCache.get(key) == null) {
        val clusterMap = Cluster.getMaps(Some(customer_id), Some(deployment_id), None, Some(true), Some(time_range))
        val cassdb = getCassandraHandle
        var arows = Array[MetricsStats]()
        clusterMap.foreach(cm => {
          val query = getServiceMetricsQuery(cm.customer_id, cm.deployment_id, cm.cluster_id, time_range, 1)
  ...
  def getServiceListFromDB(customer_id: String, deployment_id: String, time_range: TimeRange): List[NWSvcDetail] = {
    val smList = Service.getMaps(Some(customer_id), Some(deployment_id), None, None, None, None, None, None, None)

    val nsd = List(new NWSvcDetail("get_nw_svc_detail", customer_id, deployment_id,
      smList.map(x => {
        new NWSvcInfo(x.cluster_id, x.name, x.svc, x.ipver, x.proto, x.port, x.interface)
      }), time_range)
    )
    nsd
  }

  def getNWServices(customer_id: String, deployment_id: String, _arows: Array[MetricsStats], time_range: TimeRange): List[NWSvcDetail] = {
    //Replace this with the cache implementation which sorts out the issue
    val key = s"svc_${customer_id},${deployment_id}"
    if (MyCache.get(key) == null) {
      val arows = _arows.filter(_.plugin == "procstat")
      val filterList = Array[String]("service_name", "service_type", "ipver", "port", "proto", "interface")
      ...
      nwSvcList.foreach(x => {
        val cluster_id = x.getOrElse("cluster_id", "unknown").toString.stripMargin('\"')
        ...
        //Write to DB as well
        val sm = new ServiceMap(customer_id, deployment_id, cluster_id, name, svc, ipver, proto, port, interface, System.currentTimeMillis())
        Service.setMap(sm)
      })
      MyCache.put(key, "bla")
    }
  ...
  def prepareNWGraphList(customer_id: String, deployment_id: String, arows: Array[Metrics.MetricsStats], time_range: TimeRange): List[NWGraph] = {
    ...
    val nsl: Array[NWGraph] = {
      val _nsl = arows.filter(_.plugin == "nwgraph")
        .groupBy(x => {
      ...
      if (!_nsl.isEmpty) {
        _nsl.foreach(x => {
          val key = s"${x.from.host_ip},${x.to.host_ip},${x.svc_info.port},${x.ts}"
          if (!hm.contains(key)) {
            hm += (key -> x)
          }
        })
  ...
  def getFromDB(to_customer_id: String, to_deployment_id: String, tsList: Option[List[Long]],
                to_cluster_id: Option[String], to_host_ip: Option[String], from_customer_id: Option[String],
                from_deployment_id: Option[String], from_cluster_id: Option[String],
                from_host_ip: Option[String], svc_info_name: Option[String],
                svc_info_proto: Option[Int], svc_info_port: Option[Int], uhashList: Option[List[String]]
                 ): List[NWGraph] = {
    ...
    val query = {
      ...
      if (to_cluster_id.nonEmpty) str += s" AND ${CONN_STATS_COL.TO_CLUSTER_ID} = '${to_cluster_id.get}'"
      if (to_host_ip.nonEmpty) str += s" AND ${CONN_STATS_COL.TO_HOST_IP} = '${to_host_ip.get}'"
      if (from_customer_id.nonEmpty) str += s" AND ${CONN_STATS_COL.FROM_CUSTOMER_ID} = '${from_customer_id.get}'"
      if (from_deployment_id.nonEmpty) str += s" AND ${CONN_STATS_COL.FROM_DEPLOYMENT_ID} = '${from_deployment_id.get}'"
      if (from_cluster_id.nonEmpty) str += s" AND ${CONN_STATS_COL.FROM_CLUSTER_ID} = '${from_cluster_id.get}'"
      if (from_host_ip.nonEmpty) str += s" AND ${CONN_STATS_COL.FROM_HOST_IP} = '${from_host_ip.get}'"
      if (svc_info_name.nonEmpty) str += s" AND ${CONN_STATS_COL.SVC_INFO_NAME} = '${svc_info_name.get}'"
      if (svc_info_proto.nonEmpty) str += s" AND ${CONN_STATS_COL.SVC_INFO_PROTO} = ${svc_info_proto.get}"
      if (svc_info_port.nonEmpty) str += s" AND ${CONN_STATS_COL.SVC_INFO_PORT} = ${svc_info_port.get}"
      if (tsList.nonEmpty) {
    ...
    resultSet.asScala.foreach(x => {
      ...
      val to_node = {
        new Node.NodeMap(to_customer_id, to_deployment_id, to_cluster_id, to_host_ip, to_host_name,
          to_scope, to_active, to_last_modified)
      }
      ...
      val from_node = {
        new Node.NodeMap(from_customer_id, from_deployment_id, from_cluster_id, from_host_ip, from_host_name,
          from_scope, from_active, from_last_modified)
      }
      ...
      val svc_info = {
        new NWSvcInfo(svc_info_cluster_id, svc_info_name, svc_info_svc, svc_info_ipver,
          svc_info_proto, svc_info_port, svc_info_interface)
      }
      ...
      val topmost_error_stats = JacksonWrapper.deserialize[List[RequestStat]](x.getString(CONN_STATS_COL.TOPMOST_ERROR_STATS)).toArray
      val costliest_request_stats = JacksonWrapper.deserialize[List[RequestStat]](x.getString(CONN_STATS_COL.COSTLIEST_REQUEST_STATS)).toArray
      val ts = x.getTimestamp(CONN_STATS_COL.TS).getTime
      val duration = x.getLong(CONN_STATS_COL.DURATION)

      connections ::= {
        new NWGraph(to_node, from_node, svc_info, avg_rsp_size, avg_ttfb, avg_ttlb, max_ttfb, max_ttlb, error_count,
          request_count, sent_bytes, recv_bytes, topmost_error_stats, costliest_request_stats, ts, duration)
      }
    })
    cassdb.close()
    connections
  ...
  def writeToDB(nls: List[NWGraph], ttl: Option[Int]): Boolean = {
    ...
    val query_list = {
      ...
      nls.foreach(x => {
        ...
        val topmost_error_stats = JacksonWrapper.serialize(x.topmost_error_stats)
        val costliest_request_stats = JacksonWrapper.serialize(x.costliest_request_stats)
        val ts = x.ts
