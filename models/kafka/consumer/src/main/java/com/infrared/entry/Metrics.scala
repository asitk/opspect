...
object Metrics {
  ...
  case class MetricsStats(ts: Long, customer_id: String, deployment_id: String, cluster_id: String, target: String,
                          host_ip: String, host_name: String, plugin: String, classification: String,
                          high_water_mark: Double, final_score: Double, is_non_numeric: Boolean,
                          val_stats: String, vel_stats: String, tags: String, val_str: String) extends Serializable {
  ...
  case class DStats(size: Long, min_val: Double, max_val: Double, mean: Double, stddev: Double, f: Array[Int]) extends Serializable {
    override def toString: String = {
      JacksonWrapper.serialize(this)
    }
  }
      ...
      if (resultSet != null) {
        val ret = resultSet.asScala
          .map(row => {
            ...
            val classification = row.getString(METRICS_STATS_COLS.CLASSIFICATION)
            val high_water_mark = row.getDouble(METRICS_STATS_COLS.HIGH_WATER_MARK)
            ...
            new MetricsStats(ts, customer_id, deployment_id, cluster_id, target, host_ip, host_name, plugin, classification, high_water_mark, final_score,
              isNonNumeric, valStats, velStats, tags, valStr)
          })
        ret.toArray
      } else {
        Array()
  ...
  def getSystemMetricsQuery(customer_id: String, deployment_id: String, cluster_id: String, time_range: TimeRange, grain: Int): String = {
    val query = {
  ...
  def getNetworkMetricsQuery(customer_id: String, deployment_id: String, time_range: TimeRange, grain: Int): String = {
    val query = {
      val tsStr = getTimeRangeAsASetString(time_range, grain)
      s"SELECT ${METRICS_STATS_COLS.CUSTOMER_ID}, ${METRICS_STATS_COLS.DEPLOYMENT_ID}, ${METRICS_STATS_COLS.CLUSTER_ID}, " +
  ...
  def getServiceMetricsQuery(customer_id: String, deployment_id: String, cluster_id: String,
                             time_range: TimeRange, grain: Int): String = {
    val query = {
  ...
  def getLastTimestampQuery(customer_id: String, deployment_id: String): String = {
    val query = s"SELECT ${METRICS_STATS_COLS.TS} FROM ${METRICS_STATS_COLS.TABLE_NAME} " +
