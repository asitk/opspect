...
import com.infrared.entry.Metrics.{MetricsStats, _}
import com.infrared.entry.rawrecord._
import com.infrared.injest.StreamingHighLevelConsumer
...
object ProcessKafkaStream extends App {
  ...
  private def processRDD(r: RDD[RawRecord]): Unit = {
    r.foreach {
      rawrecordRDD => {} //TODO rawrecordRDD.write()
    }
  }
  ...
  object StreamDerivatesFunc {
    def updateFunc(ts: Time, key: String, value: Option[RawRecord],
                   state: State[StreamDerivativesState]): Option[(String, RawRecord)] = {
      ...
      if (currentRawRecord != null && newRawRecord != null) {
        ...
        //depth += 1
        rr.tags += (s"delta_${n}_value" -> hm.getOrElse(s"delta_${n}_value", 0).toString)
        rr.tags += (s"delta_${n}_time" -> hm.getOrElse(s"delta_${n}_time", 0).toString)
  ...
  private def avgTags(tags: Array[HashMap[String, String]]): Map[String, String] = {
        ...
        rdd2.foreach(r => {
          ...
          if (prev_ts != 0l) {
            if ((ts - prev_ts) > 60 * 1000) {
              Log.getLogger.warn(s"Falling behind in processing = ${ts - prev_ts} ms")
            }
            if (!loop_done) {
              prev_ts = ts
              loop_done = true
          ...
          val doublePoi: Vector[Double] = for (x <- r._2.map(_.valtodouble()).toVector) yield x
          ...
          fdVal(6) = r._2.map(_.valtodouble()).count(is_water_mark_present && _ >= high_water_mark)
          ...
          val valStats = new Metrics.DStats(count, doublePoi.min, doublePoi.max, peer.mean, peer.stdDev, fdVal)
          val peerMetric = new MetricsStats(ts, customer_id, deployment_id, cluster_id, target, host_ip, host_name, plugin,
            classification, high_water_mark, finalScore, r._2.last.is_non_numeric(), JacksonWrapper.serialize(valStats),
            JacksonWrapper.serialize(velStats), JacksonWrapper.serialize(tags), val_str)
  ...
  private def persistData(msl: List[Metrics.MetricsStats]): Unit = {
    ...
    try {
      val collection = sc.parallelize(msl.map(x => {
        (x.ts, Metrics.mkUhash(x), x.customer_id, x.deployment_id, x.cluster_id, x.target,
          x.host_ip, x.host_name, x.plugin, x.classification, x.high_water_mark, x.final_score,
          x.is_non_numeric, x.val_stats, x.vel_stats, x.tags, x.val_str)
      }))
      try {
        val table: String = METRICS_STATS_COLS.TABLE_NAME
        val keyspace: String = "nuvidata"

        collection.saveToCassandra(keyspace, table, SomeColumns(
        ...
        Log.getLogger.debug(s"Finished cassandra write with length = ${msl.length}")

        val fmsl = msl.filterNot(_.plugin == "nwgraph").filterNot(_.is_non_numeric).toArray
        if (fmsl.nonEmpty) {
          Metrics.writeToKairosDB(fmsl)
          Log.getLogger.debug(s"Finished kairosdb write with length = ${fmsl.length}")
        }
      } catch {
  ...
  private def initMeanStats(customer_id: String, ts: Long, invalidateCache: Boolean = false): Unit = {
    ...
    try {
      ...
      if (sqlContext.isCached(sqlTableName)) {
        ...
        val sqlStmt = s"SELECT count(*) AS count FROM ${sqlTableName} WHERE customer_id = '${customer_id}' and ts <= ${ts}"
      ...
      case e: Exception => {
        ...
        Log.getLogger.error(s"${sqlTableName} table does not exist in cache for ts <= ${ts}")

        val rdd = sc.cassandraTable(keyspace, table)
          .select(METRICS_STATS_COLS.TS, METRICS_STATS_COLS.CUSTOMER_ID, METRICS_STATS_COLS.DEPLOYMENT_ID,
            METRICS_STATS_COLS.CLUSTER_ID, METRICS_STATS_COLS.TARGET, METRICS_STATS_COLS.HOST_IP,
            METRICS_STATS_COLS.HOST_NAME, METRICS_STATS_COLS.PLUGIN, METRICS_STATS_COLS.CLASSIFICATION,
            METRICS_STATS_COLS.IS_NON_NUMERIC, METRICS_STATS_COLS.HIGH_WATER_MARK, METRICS_STATS_COLS.FINAL_SCORE,
            METRICS_STATS_COLS.VAL_STATS, METRICS_STATS_COLS.VEL_STATS, METRICS_STATS_COLS.TAGS,
            METRICS_STATS_COLS.VAL_STR
          )
          .where(s"${METRICS_STATS_COLS.CUSTOMER_ID} = '${customer_id}'")
          .where(s"${METRICS_STATS_COLS.HOST_NAME} = '*'")
          .where(s"${METRICS_STATS_COLS.TS} >= ? and ts <= ? ", (ts - windowDelta), ts)


          .map(row => {

            new Metrics.MetricsStats(
              row.getLong(METRICS_STATS_COLS.TS), row.getString(METRICS_STATS_COLS.CUSTOMER_ID),
              row.getString(METRICS_STATS_COLS.DEPLOYMENT_ID), row.getString(METRICS_STATS_COLS.CLUSTER_ID),
              row.getString(METRICS_STATS_COLS.TARGET), row.getString(METRICS_STATS_COLS.HOST_IP),
              row.getString(METRICS_STATS_COLS.HOST_NAME), row.getString(METRICS_STATS_COLS.PLUGIN),
              row.getString(METRICS_STATS_COLS.CLASSIFICATION), row.getDouble(METRICS_STATS_COLS.HIGH_WATER_MARK),
              row.getDouble(METRICS_STATS_COLS.FINAL_SCORE), row.getBoolean(METRICS_STATS_COLS.IS_NON_NUMERIC),
              row.getString(METRICS_STATS_COLS.VAL_STATS), row.getString(METRICS_STATS_COLS.VEL_STATS),
              row.getString(METRICS_STATS_COLS.TAGS), row.getString(METRICS_STATS_COLS.VAL_STR))
          })
  ...
  private def getMeanStats(customer_id: String, deployment_id: String, cluster_id: String, plugin: String, classification: String, target: String, ts: Long): Metrics.MetricsStats = {
    try {
    ...
    val rowSet: Array[Metrics.MetricsStats] = rows.map(r => {

      new Metrics.MetricsStats(ts, customer_id, deployment_id, cluster_id, target, hostip, hostname, plugin, classification,
        r.getDouble(r.fieldIndex("high_water_mark")), r.getDouble(r.fieldIndex("final_score")),
        r.getBoolean(r.fieldIndex("is_non_numeric")), r.getString(r.fieldIndex("val_stats")),
        r.getString(r.fieldIndex("vel_stats")), r.getString(r.fieldIndex("tags")), r.getString(r.fieldIndex("val_str"))
      )
  ...
  private def computeDerivatives(rl: DStream[RawRecord]): DStream[RawRecord] = {
    val mappedStream: DStream[(String, RawRecord)] = rl.map(x => {
      val hashString = s"customer_id=${x.customer_id},deployment_id=${x.deployment_id},cluster_id=${x.cluster_id},host_ip=${x.host_ip},host_name=${x.host_name}," +
        s"target=${x.target},plugin=${x.plugin},classification=${x.classification}"
      (hashString, x)
    })

    val stateDStream: DStream[(String, RawRecord)] = mappedStream.mapWithState(StateSpec.function(StreamDerivatesFunc.updateFunc _))
  ...
  private def computeInstance(rdd: RDD[RawRecord], _peer: Metrics.MetricsStats): List[Metrics.MetricsStats] = {
    ...
    if (rdd.count() > 0) {
      ...
      val rdd3 = rdd2.map(r => {
        ...
        val doublePoiValue: Vector[Double] = for (x <- r._2.map(_.valtodouble()).toVector) yield x
        ...
        val instance: MeanAndVariance = breeze.stats.meanAndVariance(doublePoiValue)
        val instance1: MeanAndVariance = breeze.stats.meanAndVariance(doublePoiValue1)
        ...
        */
        ...
        val sumScore = {
          if (_peer.final_score != 0) {
            (Metrics.computeScore(fdVal, doublePoiValue.length) - _peer.final_score) / _peer.final_score
          } else {
            0
          }
        }
        ...
        val velStats: Metrics.DStats = new Metrics.DStats(doublePoiValue1.length, doublePoiValue1.min,
          doublePoiValue1.max, instance1.mean, instance.stdDev, fdVal1)
        val valStats: Metrics.DStats = new Metrics.DStats(doublePoiValue1.length, doublePoiValue.min,
          doublePoiValue.max, instance.mean, instance.stdDev, fdVal)
        new MetricsStats(ts, customer_id, deployment_id, cluster_id, target, host_ip, host_name, plugin, classification,
          high_water_mark, finalScore, r._2.last.is_non_numeric, JacksonWrapper.serialize(valStats),
          JacksonWrapper.serialize(velStats), JacksonWrapper.serialize(tags), val_str)
      })
      rdd3.toList
    } else {
  ...
  def run(): Boolean = {
    ...
    val messages = ssc.receiverStream(new StreamingHighLevelConsumer(zooKeeper, groupId, topic))
    ...
    ssc.checkpoint("/tmp")
    val rawrecordDerivativesStream: DStream[RawRecord] = computeDerivatives(rawrecordStream)
    computeCluster(rawrecordDerivativesStream)

    val diff = {
      val ts = java.util.Calendar.getInstance(java.util.TimeZone.getTimeZone("UTC")).getTimeInMillis
