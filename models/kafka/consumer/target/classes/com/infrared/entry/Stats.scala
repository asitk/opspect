...
import com.infrared.entry.Metrics.DStats
...
object Stats {
  ...
  def findDStats(x: Array[Double]): Metrics.DStats = {
    ...
    new DStats(x.length, x.min, x.max, peer.mean, peer.stdDev, fpMap.map(_.length))
  }
  ...
  def findPeerMean(x: Array[Double]): (MeanAndVariance, Array[List[Double]]) = {
    ...
    do {
      ...
      Log.getLogger.debug(s"FP Means are ${JacksonWrapper.serialize(peer)} and ntries = ${ntries}")
  ...
  def findFPMap(x: Array[Double], peer: MeanAndVariance): Array[List[Double]] = {
    ...
    for (x <- doublePoi) {
      val index: Int = {
        ...
        else if (Metrics.isInRange(x, peer.mean - 2 * peer.stdDev, peer.mean - peer.stdDev)) {
          1
        }
        ...
        else if (Metrics.isInRange(x, peer.mean + peer.stdDev, peer.mean + 2 * peer.stdDev)) {
          4
        }
        else if (x >= peer.mean + 2 * peer.stdDev) {
          5
