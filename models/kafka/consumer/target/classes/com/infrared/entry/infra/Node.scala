...
object Node extends Group{
  case class NodeMap(customer_id : String, deployment_id : String, cluster_id : String, host_ip : String, host_name : String,
                        scope: String, active : Boolean, last_modified : Long) extends Serializable {
  ...
  def getMaps(customer_id: Option[String], deployment_id : Option[String], cluster_id : Option[String],
              host_ip : Option[String], active : Option[Boolean], time_range: Option[TimeRange]) : List[NodeMap] = {
    ...
    val query = s"SELECT ${NODE_MAP_COLS.CUSTOMER_ID}, ${NODE_MAP_COLS.DEPLOYMENT_ID}, ${NODE_MAP_COLS.CLUSTER_ID}, " +
    ...
    val node_map = {
      if (resultSet != null) {
        val node_map = resultSet.asScala
          .filter(tr == null || _.getTimestamp(NODE_MAP_COLS.TS).getTime <= tr.end)
          .filter(cust_id_filtered == _.getString(NODE_MAP_COLS.CUSTOMER_ID) || cust_id_filtered == null)
          .filter(deployment_id_filtered == _.getString(NODE_MAP_COLS.DEPLOYMENT_ID) || deployment_id_filtered == null)
          .filter(cluster_id_filtered == _.getString(NODE_MAP_COLS.CLUSTER_ID) || cluster_id_filtered == null)
          .filter(host_ip_filtered == _.getString(NODE_MAP_COLS.HOST_IP) || host_ip_filtered == null)
          .groupBy(row => {
          ...
          .filter(active_filtered && _.getBool(NODE_MAP_COLS.ACTIVE) || !active_filtered)
          .map(row => {
            new NodeMap(row.getString(NODE_MAP_COLS.CUSTOMER_ID), row.getString(NODE_MAP_COLS.DEPLOYMENT_ID),
              row.getString(NODE_MAP_COLS.CLUSTER_ID), row.getString(NODE_MAP_COLS.HOST_IP),
              row.getString(NODE_MAP_COLS.HOST_NAME), row.getString(NODE_MAP_COLS.SCOPE),
              row.getBool(NODE_MAP_COLS.ACTIVE), row.getTimestamp(NODE_MAP_COLS.TS).getTime)
          }).toList
  ...
  def setMap(item : NodeMap) : Boolean = {
    ...
    val query = s"INSERT INTO ${NODE_MAP_COLS.TABLE_NAME}(${NODE_MAP_COLS.CUSTOMER_ID}, ${NODE_MAP_COLS.DEPLOYMENT_ID}," +
  ...
  def getTimelineMarkers(customer_id :String, deployment_id : String, cluster_id : String, host_ip : String, time_range : TimeRange) : NodeTimelineMarker = {
    val nodeMap = Node.getMaps(Some(customer_id), Some(deployment_id), Some(cluster_id), Some(host_ip), Some(true), Some(time_range))
    if (nodeMap.length == 1) {
      val hour_marker_list: List[Marker] = {
        val ssh = new SummaryStatsHourly(customer_id, deployment_id)
        val query = ssh.prepareTimelineQuery(cluster_id, nodeMap.head.host_name, time_range)
        val cassdb = getCassandraHandle
        val resultSet = cassdb.executeQuery(query)
      ...
      new NodeTimelineMarker("get_node_timeline_markers", customer_id, deployment_id,
        cluster_id, host_ip, nodeMap.head.host_name,
        nodeMap.head.scope, nodeMap.head.active, hour_marker_list ::: minute_marker_list, time_range)
    } else {
      throw new Exception(s"Customer_id = ${customer_id} with deployment_id = ${deployment_id}  with cluster_id = ${cluster_id} " +
  ...
  case class NodeDetailWithoutTimeRange(host_name: String,  host_ip : String,
  ...
  case class NodeDetail(method : String,customer_id : String, deployment_id : String,  cluster_id : String, host_name: String,  host_ip : String,
                              scope : String, active : Boolean,
                              thermal : Int, thermal_count : Map[Int, Int],
  ...
  case class NodeSnapshot(method : String,customer_id : String, deployment_id : String,  cluster_id : String, host_name: String,  host_ip : String,
                          scope : String, active : Boolean,
                          thermal: Int, thermal_count: Map[Int, Int],
  ...
  case class NodeConnection(method: String, customer_id: String, deployment_id: String, cluster_id: String, host_name: String, host_ip: String,
                            scope: String, active: Boolean,
  ...
  case class NodeService(method: String, customer_id: String, deployment_id: String, cluster_id: String, host_name: String, host_ip: String,
                            scope: String, active: Boolean,
  ...
  case class NodeTimelineMarker(method : String, customer_id : String, deployment_id : String, cluster_id: String,  host_ip : String,
                                  host_name : String, scope : String, active : Boolean, markers : List[Marker],
                                   time_range : TimeRange) extends Serializable {
    override def toString: String = {
      JacksonWrapper.serialize(this)
    }
  ...
  def getDetails(customer_id :String, deployment_id : String, cluster_id : String, host_ip : String, time_range : TimeRange) : List[NodeDetail] = {
    ...
    deploymentMap.foreach(dm => {
      val clusterMap = Cluster.getMaps(Some(dm.customer_id), Some(dm.deployment_id), Some(cluster_id), Some(true), Some(time_range))
      clusterMap.foreach(cm => {
        val cluster_replicated = cm.replicated
        val nodeMap = Node.getMaps(Some(dm.customer_id), Some(dm.deployment_id), Some(cluster_id), Some(host_ip), Some(true), Some(time_range))
        ni = nodeMap.map(nm => {
          val thermal_structs_hourly: List[(Long, Int, String)] = {
            val cassdb = getCassandraHandle
            val ssh = new SummaryStatsHourly(customer_id, deployment_id)
            val query = ssh.prepareDetailsQuery(nm.cluster_id, nm.host_name, time_range)
            val resultSet = cassdb.executeQuery(query)
          ...
          val qtcounts = getThermalStats((thermal_structs_hourly ::: thermal_structs_minute).toArray, cluster_replicated)
          new NodeDetail("get_node_detail", nm.customer_id, nm.deployment_id, nm.cluster_id, nm.host_name, nm.host_ip,
            nm.scope, nm.active, qtcounts._1, qtcounts._2, time_range)
        })
      })
    })
  ...
  def getSnapshot(customer_id :String, deployment_id : String, cluster_id : String, host_ip : String, time_range : TimeRange) : List[NodeSnapshot] = {
    val deploymentMap = Deployment.getMaps(Some(customer_id), Some(deployment_id), None, Some(time_range))
      ...
      clusterMap.foreach(cm => {
        ...
        nd = nodeMap.map(nm => {
          val thermal_structs_hourly: List[(Long, Int, String, String, String, String)] = {
            val cassdb = getCassandraHandle
            val ssh = new SummaryStatsHourly(customer_id, deployment_id)
            val query = ssh.prepareSnapshotQuery(nm.cluster_id, nm.host_name, time_range)
            val resultSet = cassdb.executeQuery(query)
          ...
          new NodeSnapshot("get_node_snapshot", nm.customer_id, nm.deployment_id, nm.cluster_id, nm.host_name, nm.host_ip,
            nm.scope, nm.active, qtcounts._1, qtcounts._2, statsList.sortWith(_.contribution.rank < _.contribution.rank),
  ...
  def getConnection(customer_id: String, deployment_id: String, cluster_id: String, host_ip: String, time_range: TimeRange): List[NodeConnection] = {
    val deploymentMap = Deployment.getMaps(Some(customer_id), Some(deployment_id), None, Some(time_range))
      ...
      clusterMap.foreach(cm => {
        ...
        nd = nodeMap.map(nm => {
          val connections_structs_hourly: List[(Long, String)] = {
            val cassdb = getCassandraHandle
            val ssh = new SummaryStatsHourly(customer_id, deployment_id)
            val query = ssh.prepareConnectionQuery(nm.cluster_id, nm.host_name, time_range)
            val resultSet = cassdb.executeQuery(query)
          ...
          if (connectionStructs.nonEmpty) {
            ...
            new NodeConnection("get_node_connection", nm.customer_id, nm.deployment_id, nm.cluster_id, nm.host_name, nm.host_ip,
              nm.scope, nm.active, connList, time_range)
          } else {
            null
          }
        }).filterNot(_ == null)
  ...
  def getService(customer_id: String, deployment_id: String, cluster_id: String, host_ip: String, time_range: TimeRange): List[NodeService] = {
    val deploymentMap = Deployment.getMaps(Some(customer_id), Some(deployment_id), None, Some(time_range))
      ...
      clusterMap.foreach(cm => {
        ...
        nd = nodeMap.map(nm => {
          val services_structs_hourly: List[(Long, String)] = {
            val cassdb = getCassandraHandle
            val ssh = new SummaryStatsHourly(customer_id, deployment_id)
            val query = ssh.prepareServiceQuery(nm.cluster_id, nm.host_name, time_range)
            val resultSet = cassdb.executeQuery(query)
          ...
          if (serviceStructs.nonEmpty) {
            ...
            new NodeService("get_node_service", nm.customer_id, nm.deployment_id, nm.cluster_id, nm.host_name, nm.host_ip,
              nm.scope, nm.active, servicesList, time_range)
          } else {
            null
          }
        }).filterNot(_ == null)
