...
object CassandraDB {
  ...
  private var instance = scala.collection.mutable.HashMap[String,(CassDB, Int)]()
    ...
    val session : Session = {
      ...
      if (!inited || cluster.isClosed) {
        ...
        cluster = {
          ...
          Cluster.builder().addContactPointsWithPorts(addressList.asJavaCollection)
      ...
      try {
        ...
        do {
          if (result.isFullyFetched || ntries == 0) {
            ...
            Log.getLogger.debug(s"Not fully fetched yet - ${ntries} tries remaining")
            Thread.sleep(100L)
            ntries -= 1
          }
        } while (!matched)
        ...
        case e: Exception => {
          Log.getLogger.error(s"Got exception ${e.getMessage} on query ${query}")
